...
class MemoryMappedDataset(Dataset):
    ...
    Directory structure expected:
        ...
        labels_pooled_{text_model}.npy           : float32 [N, pooled_dim]
    ...
    def __init__(
        ...
        _init_224_path = f"{data_path}/initial_224.npy"
        _tgt_224_path = f"{data_path}/target_224.npy"
        self.initial_224 = np.load(_init_224_path, mmap_mode='r') if os.path.exists(_init_224_path) else None
        self.target_224 = np.load(_tgt_224_path, mmap_mode='r') if os.path.exists(_tgt_224_path) else None
        ...
        # Pooled text embeddings for AdaLN conditioning
        self.label_pooled = np.load(f"{data_path}/labels_pooled_{text_model}.npy", mmap_mode='r')
        ...
        if self.initial_224 is not None:
            size_arrays.append(len(self.initial_224))
        if self.target_224 is not None:
            size_arrays.append(len(self.target_224))
        max_valid = min(size_arrays)
    ...
    def __getitem__(self, idx):
        """Returns (x0_224, x0_embed, xt_224, xt_embed, c_txt, c_hidden, c_attn_mask, c_pooled).
        ...
        - c_txt:        string
        - c_hidden:     tensor float32 [77, text_dim]  (per-token hidden states)
        - c_attn_mask:  tensor int32   [77]            (1=real token, 0=padding)
        - c_pooled:     tensor float32 [pooled_dim]    (pooled text for AdaLN)
        """
        actual_idx = self.indices[idx]
        
        c_pooled = torch.from_numpy(self.label_pooled[actual_idx].copy())

        x0_224 = torch.from_numpy(self.initial_224[actual_idx].copy()) if self.initial_224 is not None else torch.zeros(3, 224, 224)
        xt_224 = torch.from_numpy(self.target_224[actual_idx].copy()) if self.target_224 is not None else torch.zeros(3, 224, 224)
...
def mmap_collate_fn(batch):
    ...
    c_pooled = torch.stack([b[7] for b in batch])     # [B, pooled_dim]
    
    return x0_224, x0_embed, xt_224, xt_embed, c_txt, c_hidden, c_attn_mask, c_pooled
