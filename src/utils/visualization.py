...
def visualize_flowdit_samples(
    ...
    pooled_text_batch: torch.Tensor = None,
):
    """
    Generate and visualize DiT samples.
    ...
    pooled_vis = pooled_text_batch[:n_vis].to(device) if pooled_text_batch is not None else None

    # Fall back to decoding from latents when raw 224 images aren't in the dataset
    # (some mmap datasets — e.g. subgoal — skip storing initial_224/target_224)
    def _is_blank(t):
        return isinstance(t, torch.Tensor) and float(t.abs().sum()) == 0.0

    if decode_fn is not None and _is_blank(x0_vis):
        x0_vis = decode_fn(z_init / scale_factor).detach().cpu().clamp(0, 1)
    if decode_fn is not None and _is_blank(xt_vis):
        xt_vis = decode_fn(z_target_gt / scale_factor).detach().cpu().clamp(0, 1)

    # Generate (bf16 autocast for Flash Attention + speed)
    with torch.autocast('cuda', dtype=torch.bfloat16):
        z_generated = model.sample_euler(
            z_init, text_hidden_vis, text_mask=text_mask_vis,
            pooled_text_emb=pooled_vis,
...
def visualize_distill_samples(
    ...
    """Side-by-side teacher (3-fwd CFG) vs student (1-fwd, distilled) samples.

    4-row × N-col grid: initial / target / teacher / student.
    Requires a Theia decoder (no cosine-sim fallback — distillation visualizations
    are only useful when the latent images can be rendered).
    """
    teacher_model.eval()
    student_model.eval()
    ...
    z_init   = x0_embed_batch[:n_vis].to(device) * scale_factor
    z_target = xt_embed_batch[:n_vis].to(device) * scale_factor
    text_hidden_vis = text_hidden_batch[:n_vis].to(device)
    text_mask_vis   = text_mask_batch[:n_vis].to(device)
    text_txt_vis    = text_txt_batch[:n_vis]
    pooled_vis = pooled_text_batch[:n_vis].to(device) if pooled_text_batch is not None else None
    ...
    if decode_fn is not None and _is_blank(x0_vis):
        x0_vis = decode_fn(z_init / scale_factor).detach().cpu().clamp(0, 1)
    if decode_fn is not None and _is_blank(xt_vis):
        xt_vis = decode_fn(z_target / scale_factor).detach().cpu().clamp(0, 1)

    with torch.autocast('cuda', dtype=torch.bfloat16):
        z_teacher = teacher_model.sample_euler(
            z_init, text_hidden_vis, text_mask=text_mask_vis,
            pooled_text_emb=pooled_vis, num_steps=num_steps,
            context_cfg_scale=context_cfg_scale,
            prompt_cfg_scale=prompt_cfg_scale,
        )
        z_student = student_model.sample_euler(
            z_init, text_hidden_vis, text_mask=text_mask_vis,
            pooled_text_emb=pooled_vis, num_steps=num_steps,
        )

    teach_imgs = decode_fn(z_teacher / scale_factor).detach().cpu().clamp(0, 1)
    stud_imgs  = decode_fn(z_student / scale_factor).detach().cpu().clamp(0, 1)

    fig, axes = plt.subplots(4, n_vis, figsize=(n_vis * 2, 8), squeeze=False)
    row_labels = ['Initial', 'Target', 'Teacher (3-fwd CFG)', 'Student (1-fwd)']
    ...
    for i in range(n_vis):
        ...
        axes[0, i].imshow(_to_hwc(x0_vis[i]));   axes[0, i].axis('off')
        axes[1, i].imshow(_to_hwc(xt_vis[i]));   axes[1, i].axis('off')
        axes[2, i].imshow(_to_hwc(teach_imgs[i])); axes[2, i].axis('off')
        axes[3, i].imshow(_to_hwc(stud_imgs[i]));  axes[3, i].axis('off')
